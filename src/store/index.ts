...
import { COMMON, log, retry, utils } from '@/utils';
import { YENClient } from 'yen-sdk';
import { toRaw } from 'vue';

export interface Storage {
  offerNumberList: Array<number>;
  getOfferAmountLimit: number;
  getAnswerBlockAmountLimit: number;
}
...
export interface Offer {
  offerData: OfferRewardModel.OfferData;
  offerPublishedEvent?: OfferRewardModel.OfferPublishedEvent;
  offerFinishedEvent?: OfferRewardModel.OfferFinishedEvent;
  answerPublishedEventMap?: { [blockId: number]: Array<OfferRewardModel.AnswerPublishedEvent> };
}

export interface Publisher {
  publisherData: OfferRewardModel.PublisherData;
  offerIdList?: number[];
}

export interface Async {
  offerMap: { [offerId: number]: Offer };
  publisherMap: { [address: string]: Publisher };
  offerReward: {
    offerLength: number;
    blockSkip: number;
    sortLength: number;
    firstValueSortOfferId: number;
    firstFinishSortOfferId: number;
    valueSortOfferIdList: number[],
    finishSortOfferIdList: number[],
    offerIdList: number[],
  };
}
...
const state: State = {
  storage: {
    offerNumberList: [],
    getOfferAmountLimit: 5,
    getAnswerBlockAmountLimit: 5,
  },
  sync: {
  ...
  async: {
    offerReward: {
      ...
      blockSkip: 5000,
      sortLength: 0,
      firstValueSortOfferId: 0,
      firstFinishSortOfferId: 0,
      valueSortOfferIdList: [],
      finishSortOfferIdList: [],
      offerIdList: [],
    },
    offerMap: {},
    publisherMap: {},
...
const actions: ActionTree<State, State> = {
  async start({ dispatch }, { chainId }) {
    try {
      await dispatch('setSync', { chainId });
      await dispatch('watchStorage');
      state.async.offerReward.sortLength = await retry(toRaw(state.sync.ether.offerReward).getSortLength.bind(toRaw(state.sync.ether.offerReward)), retryTime);
      await Promise.all([
  ...
  async setSync({ state, dispatch }, { chainId }) {
    state.sync.isMobile = utils.is.mobile();
    await toRaw(state.sync.ether).load(chainId);
